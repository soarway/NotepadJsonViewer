...
public:
    ...
    auto FormatCode(const std::string &jsonText, LE le, LF lf, char indentChar, unsigned indentLen) -> const Result;
    ...
    template <unsigned format, typename Handler>
    auto ParseCode(const std::string &codeText, rj::StringBuffer &sb, Handler &handler) -> const Result;

    char *getNextLine(char *buf, int buflen, int *datalen, char *ptr);
    std::string parse_code_text(char *codeText);
};
...
template <unsigned flgBase, typename Handler>
inline auto JsonHandler::ParseCode(const std::string &codeText, rj::StringBuffer &sb, Handler &handler) -> const Result
{
    Result retVal {};
    (void *)&handler;
    (void *)&sb;
    ...
    const char *ptr = codeText.c_str();
    size_t      dlen = strlen(ptr);
    char *str = (char*)malloc(dlen+1);
    memset(str, 0, dlen + 1);
    memcpy(str, ptr, dlen);

    std::string rs = parse_code_text(str);
