...
void JsonViewDlg::FormatCode()
{
    const auto selectedText                    = m_Editor->GetJsonText();
    const auto [le, lf, indentChar, indentLen] = GetFormatSetting();

    Result res = JsonHandler(m_pSetting->parseOptions).FormatCode(selectedText, le, lf, indentChar, indentLen);
...
void JsonViewDlg::HandleTabActivated()
{
    ...
    if (bIsVisible && m_Editor->IsJsonFile())
    {
        ...
        {
            FormatJson();//这里不用模仿，这里是自动格式化json的设置
        }
    }
}
